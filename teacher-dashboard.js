...
const TeacherDashboard = {
    ...
    setupEventListeners() {
        document.getElementById('catalogSearch')?.addEventListener('input', Utils.debounce(() => this.filterCatalog(), 300));
        document.getElementById('catalogCategory')?.addEventListener('change', () => this.filterCatalog());
    },

    switchTab(tabName) {
        document.querySelectorAll('#teacherDashboard .tab').forEach(tab => tab.classList.remove('active'));
        event.target.classList.add('active');

        document.querySelectorAll('#teacherDashboard .tab-content').forEach(content => content.classList.add('hidden'));
        const tabContent = document.getElementById(`teacher-tab-${tabName}`);
        if (tabContent) {
            tabContent.classList.remove('hidden');
        }

        if (tabName === 'catalog') this.loadCatalog();
        if (tabName === 'reserved') this.loadReservedBooks();
        if (tabName === 'issued') this.loadIssuedBooks();
    },
    ...
    filterCatalog() {
        ...
        const container = document.getElementById('teacherCatalogList');
        if (!container) return;
                ...
                <div class="book-card">
                    ...
                    <div class="actions">
                        <button class="btn ${isBookmarked ? 'btn-warning' : 'btn-secondary'} btn-sm" 
                                onclick="TeacherDashboard.toggleBookmark('${book.id}')">
    ...
    loadReservedBooks() {
        // For now, just show an empty state
        const container = document.getElementById('reservedBooksList');
        if (!container) return;
        container.innerHTML = '<div class="empty-state">No reserved books</div>';
    },
    ...
    loadIssuedBooks() {
        ...
        if (activeTransactions.length === 0) {
            container.innerHTML = '<div class="empty-state">No issued books</div>';
            return;
        }
